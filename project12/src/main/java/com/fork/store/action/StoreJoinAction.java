...
//		}
		...
		int result = dao.storeJoin(dto, 1); // 1대신 c_no 를 넣기
		
		// 페이지 이동
		ActionForward forward = new ActionForward(); // 테스트 후에는 삭제할 것
		if(result == 1) {
			System.out.println(" M : 가게 등록 성공!"); // 
			forward.setPath("./storeJoin.st"); // 점주 마이페이지로 연결할 것
			forward.setRedirect(true);
		} else if(result == -1 ) {
